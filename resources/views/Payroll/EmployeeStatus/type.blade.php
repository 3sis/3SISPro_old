                ...
                <div class="modal-dialog modal-dialog-centered 3SISPro-modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                        ...
                        <form id="singleLevelDataEntryForm" autocomplete="off" 
                            method="post" action="{{ route('type.postData') }}">
                                        ...
                                        <div class="tab-pane fade show active" id="animated-underline-home" role="tabpanel" 
                                            ...
                                            <div class="container-fluid">
                                                <div class="form-group">
                                                    <input type="hidden" name="ESTYHUniqueId" id="ESTYHUniqueId" class="form-control" />
                                                </div>
                                                <div class="row mt-0">
                                                    <div class="col-md-6">
                                                        <div class="form-group">
                                                            <label>Type Id</label>
                                                            <span class="error-text ESTYHTypeId_error text-danger" style="float: right;"></span>
                                                            <input type="text" name="ESTYHTypeId" id="ESTYHTypeId" class="form-control few-options" maxlength="10" placeholder="Type Id" style="opacity: 1;" />
                                                        </div>
                                                    </div>
                                                    <div class="col-md-6">
                                                        <div class="form-group">
                                                            <label>Description</label>
                                                            <span class="error-text ESTYHDesc1_error text-danger" style="float: right;"></span>
                                                            <input type="text" name="ESTYHDesc1" id="ESTYHDesc1" class="form-control few-options" maxlength="100" placeholder="Type Description" />
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="form-group">
                                                    <label>Description2</label>
                                                    <span class="error-text ESTYHDesc2_error text-danger" style="float: right;"></span>
                                                    <textarea
                                                        name="ESTYHDesc2"
                                                        id="ESTYHDesc2"
                                                ...
                                                <div class="form-group">
                                                    <label>BI Desc</label>
                                                    <span class="error-text ESTYHBiDesc_error text-danger" style="float: right;"></span>
                                                    <input type="text" name="ESTYHBiDesc" id="ESTYHBiDesc" class="form-control few-options" maxlength="100" placeholder="BI Description" />
                                                </div>
                                            </div>
                                        </div>
                                        <div class="tab-pane fade" id="animated-underline-profile" role="tabpanel" 
                                            ...
                                            <div class="media">
                                                <div class="media-body">
                                                    <div class="form-group">
                                                        <label> User</label>
                                                        <input type="text" name="ESTYHUser" id="ESTYHUser" 
                                                        class="form-control col-sm-6" readonly/>
                                                    </div>
                                                    <div class="form-group">
                                                        <label> Created Date</label>
                                                        <input type="date" name="ESTYHLastCreated" id="ESTYHLastCreated" 
                                                        class="form-control col-sm-6" readonly/>
                                                    </div>
                                                    <div class="form-group">
                                                        <label> Updated Date</label>
                                                        <input type="date" name="ESTYHLastUpdated" id="ESTYHLastUpdated" 
                                                        class="form-control col-sm-6" readonly />
                                                    </div>                                                        
                                                </div>
                                            </div>
...
<script>
    $(document).ready(function(){
        ...
        $('#html5-extension3SIS').DataTable( {
            ...
            processing: true,
            serverSide: true,                
            "ajax": "{{ route('type.browserData')}}",        
            "columns":[                    
                {data: "ESTYHTypeId"},
                {data: "ESTYHDesc1"},
                {data: "ESTYHDesc2"},                
                {data: "action", orderable:false, searchable: false},
                {data: "ESTYHUser", "visible": false},
                {data: "ESTYHLastUpdated", "visible": false},
                {data: "ESTYHUniqueId", "visible": false},
            ],
            "columnDefs": [
                { "width": "10%", "targets": 0 },
                { "width": "25%", "targets": 1 },
        ...
        $('#add_Data').click(function(){                    
            $("#ESTYHTypeId").attr("readonly", false);                
        ...
        $(document).on('click', '.edit', function(){
            ...
            $.ajax({
                ...
                {
                    var lastCreated = formattedDate(new Date(data.ESTYHLastCreated));
                    var lastUpdated = formattedDate(new Date(data.ESTYHLastUpdated));
                    $('#ESTYHUniqueId').val(data.ESTYHUniqueId);
                    $('#ESTYHTypeId').val(data.ESTYHTypeId);
                    $('#ESTYHDesc1').val(data.ESTYHDesc1);
                    $('#ESTYHDesc2').val(data.ESTYHDesc2);
                    $('#ESTYHBiDesc').val(data.ESTYHBiDesc);
                    $('#ESTYHUser').val(data.ESTYHUser);
                    $('#ESTYHLastCreated').val(lastCreated);
                    $('#ESTYHLastUpdated').val(lastUpdated);
                    $("#ESTYHTypeId").attr("readonly", true);                       
        ...
        $(document).on('click', '.delete', function(){
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    $deleteMessage3SIS = fnSingleLevelDeleteConfirmation($modalTitle, data.ESTYHTypeId, data.ESTYHDesc1);   
        ...
        $('#Undelete_Data').click(function(){                    
            ...
            destroy: true,                    
            // CopyChange                    
            "ajax": "{{ route('type.browserDeletedRecords')}}",
                "columns":[
                    // CopyChange
                    {data: "ESTYHTypeId"},
                    {data: "ESTYHDesc1"},
        ...
        $(document).on('click', '.restore', function(){
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    $restoreMessage3SIS = fnSingleLevelRestoreConfirmation($modalTitle, data.ESTYHTypeId, '');   
            ...
            $(document).on('click', '.confirmrestore', function(){
                $.ajax({
                    url:"{{route('type.restoreDeletedRecords')}}",
                    mehtod:"get",
                    data:{id:DeletedUniqueId},
                    success:function(data)
                    {                       
