                ...
                <div class="modal-dialog modal-dialog-centered 3SISPro-modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                        ...
                        <form id="singleLevelDataEntryForm" autocomplete="off" 
                            method="post" action="{{ route('race.postData') }}">
                                        ...
                                        <div class="tab-pane fade show active" id="animated-underline-home" role="tabpanel" 
                                            ...
                                            <div class="container-fluid">
                                                <div class="form-group">
                                                    <input type="hidden" name="GMRAHUniqueId" id="GMRAHUniqueId" class="form-control" />
                                                </div>
                                                <div class="row mt-0">
                                                    <div class="col-md-6">
                                                        <div class="form-group">
                                                            <label>Race Id</label>
                                                            <span class="error-text GMRAHRaceId_error text-danger" style="float: right;"></span>
                                                            <input type="text" name="GMRAHRaceId" id="GMRAHRaceId" class="form-control few-options" maxlength="10" placeholder="Race Id" style="opacity: 1;" />
                                                        </div>
                                                    </div>
                                                    <div class="col-md-6">
                                                        <div class="form-group">
                                                            <label>Description</label>
                                                            <span class="error-text GMRAHDesc1_error text-danger" style="float: right;"></span>
                                                            <input type="text" name="GMRAHDesc1" id="GMRAHDesc1" class="form-control few-options" maxlength="100" placeholder="Race Description" />
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="form-group">
                                                    <label>Description2</label>
                                                    <span class="error-text GMRAHDesc2_error text-danger" style="float: right;"></span>
                                                    <textarea
                                                        name="GMRAHDesc2"
                                                        id="GMRAHDesc2"
                                                ...
                                                <div class="form-group">
                                                    <label>BI Desc</label>
                                                    <span class="error-text GMRAHBiDesc_error text-danger" style="float: right;"></span>
                                                    <input type="text" name="GMRAHBiDesc" id="GMRAHBiDesc" class="form-control few-options" maxlength="100" placeholder="BI Description" />
                                                </div>
                                            </div>
                                        </div>
                                        <div class="tab-pane fade" id="animated-underline-profile" role="tabpanel" 
                                            ...
                                            <div class="media">
                                                <div class="media-body">
                                                    <div class="form-group">
                                                        <label> User</label>
                                                        <input type="text" name="GMRAHUser" id="GMRAHUser" 
                                                        class="form-control col-sm-6" readonly/>
                                                    </div>
                                                    <div class="form-group">
                                                        <label> Created Date</label>
                                                        <input type="date" name="GMRAHLastCreated" id="GMRAHLastCreated" 
                                                        class="form-control col-sm-6" readonly/>
                                                    </div>
                                                    <div class="form-group">
                                                        <label> Updated Date</label>
                                                        <input type="date" name="GMRAHLastUpdated" id="GMRAHLastUpdated" 
                                                        class="form-control col-sm-6" readonly />
                                                    </div>                                                        
                                                </div>
                                            </div>
...
<script>
    $(document).ready(function(){
        ...
        $('#html5-extension3SIS').DataTable( {
            ...
            processing: true,
            serverSide: true,                
            "ajax": "{{ route('race.browserData')}}",        
            "columns":[                    
                {data: "GMRAHRaceId"},
                {data: "GMRAHDesc1"},
                {data: "GMRAHDesc2"},                
                {data: "action", orderable:false, searchable: false},
                {data: "GMRAHUser", "visible": false},
                {data: "GMRAHLastUpdated", "visible": false},
                {data: "GMRAHUniqueId", "visible": false},
            ],
            "columnDefs": [
                { "width": "10%", "targets": 0 },
                { "width": "25%", "targets": 1 },
        ...
        $('#add_Data').click(function(){                    
            $("#GMRAHRaceId").attr("readonly", false);                
        ...
        $(document).on('click', '.edit', function(){
            var id = $(this).attr('id');
            $.ajax({
                url: "{{route('race.fetchData')}}",
                method: 'GET',
                data: {id:id},
                dataType: 'json',
                success: function(data)
                {
                    var lastCreated = formattedDate(new Date(data.GMRAHLastCreated));
                    var lastUpdated = formattedDate(new Date(data.GMRAHLastUpdated));
                    $('#GMRAHUniqueId').val(data.GMRAHUniqueId);
                    $('#GMRAHRaceId').val(data.GMRAHRaceId);
                    $('#GMRAHDesc1').val(data.GMRAHDesc1);
                    $('#GMRAHDesc2').val(data.GMRAHDesc2);
                    $('#GMRAHBiDesc').val(data.GMRAHBiDesc);
                    $('#GMRAHUser').val(data.GMRAHUser);
                    $('#GMRAHLastCreated').val(lastCreated);
                    $('#GMRAHLastUpdated').val(lastUpdated);
                    $("#GMRAHRaceId").attr("readonly", true);                       
        ...
        $(document).on('click', '.delete', function(){
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    $deleteMessage3SIS = fnSingleLevelDeleteConfirmation($modalTitle, data.GMRAHRaceId, data.GMRAHDesc1);   
        ...
        $('#Undelete_Data').click(function(){                    
            ...
            destroy: true,                    
            // CopyChange                    
            "ajax": "{{ route('race.browserDeletedRecords')}}",
                "columns":[
                    // CopyChange
                    {data: "GMRAHRaceId"},
                    {data: "GMRAHDesc1"},
        ...
        $(document).on('click', '.restore', function(){
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    $restoreMessage3SIS = fnSingleLevelRestoreConfirmation($modalTitle, data.GMRAHRaceId, '');   
            ...
            $(document).on('click', '.confirmrestore', function(){
                $.ajax({
                    url:"{{route('race.restoreDeletedRecords')}}",
                    mehtod:"get",
                    data:{id:DeletedUniqueId},
                    success:function(data)
                    {                       
