                ...
                <div class="modal-dialog modal-dialog-centered 3SISPro-modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                        ...
                        <form id="singleLevelDataEntryForm" autocomplete="off" 
                            method="post" action="{{ route('skills.postData') }}">
                                        ...
                                        <div class="tab-pane fade show active" id="animated-underline-home" role="tabpanel" 
                                            ...
                                            <div class="container-fluid">
                                                <div class="form-group">
                                                    <input type="hidden" name="CMSKHUniqueId" id="CMSKHUniqueId" class="form-control" />
                                                </div>
                                                <div class="row mt-0">
                                                    <div class="col-md-6">
                                                        <div class="form-group">
                                                            <label>Skills Id</label>
                                                            <span class="error-text CMSKHSkillId_error text-danger" style="float: right;"></span>
                                                            <input type="text" name="CMSKHSkillId" id="CMSKHSkillId" class="form-control few-options" maxlength="10" placeholder="Skills Id" style="opacity: 1;" />
                                                        </div>
                                                    </div>
                                                    <div class="col-md-6">
                                                        <div class="form-group">
                                                            <label>Description</label>
                                                            <span class="error-text CMSKHDesc1_error text-danger" style="float: right;"></span>
                                                            <input type="text" name="CMSKHDesc1" id="CMSKHDesc1" class="form-control few-options" maxlength="100" placeholder="Skills Description" />
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="form-group">
                                                    <label>Description2</label>
                                                    <span class="error-text CMSKHDesc2_error text-danger" style="float: right;"></span>
                                                    <textarea
                                                        name="CMSKHDesc2"
                                                        id="CMSKHDesc2"
                                                        ...
                                                        name="alloptions"
                                                        id="alloptions1"
                                                        placeholder="Skills Description2"
                                                ...
                                                <div class="form-group">
                                                    <label>BI Desc</label>
                                                    <span class="error-text CMSKHBiDesc_error text-danger" style="float: right;"></span>
                                                    <input type="text" name="CMSKHBiDesc" id="CMSKHBiDesc" class="form-control few-options" maxlength="100" placeholder="BI Description" />
                                                </div>
                                            </div>
                                        </div>
                                        <div class="tab-pane fade" id="animated-underline-profile" role="tabpanel" 
                                            ...
                                            <div class="media">
                                                <div class="media-body">
                                                    <div class="form-group">
                                                        <label> User</label>
                                                        <input type="text" name="CMSKHUser" id="CMSKHUser" 
                                                        class="form-control col-sm-6" readonly/>
                                                    </div>
                                                    <div class="form-group">
                                                        <label> Created Date</label>
                                                        <input type="date" name="CMSKHLastCreated" id="CMSKHLastCreated" 
                                                        class="form-control col-sm-6" readonly/>
                                                    </div>
                                                    <div class="form-group">
                                                        <label> Updated Date</label>
                                                        <input type="date" name="CMSKHLastUpdated" id="CMSKHLastUpdated" 
                                                        class="form-control col-sm-6" readonly />
                                                    </div>                                                        
                                                </div>
                                            </div>
...
<script>
    $(document).ready(function(){
        ...
        $('#html5-extension3SIS').DataTable( {
            ...
            processing: true,
            serverSide: true,                
            "ajax": "{{ route('skills.browserData')}}",        
            "columns":[                    
                {data: "CMSKHSkillId"},
                {data: "CMSKHDesc1"},
                {data: "CMSKHDesc2"},                
                {data: "action", orderable:false, searchable: false},
                {data: "CMSKHUser", "visible": false},
                {data: "CMSKHLastUpdated", "visible": false},
                {data: "CMSKHUniqueId", "visible": false},
            ],
            "columnDefs": [
                { "width": "10%", "targets": 0 },
                { "width": "25%", "targets": 1 },
        ...
        $('#add_Data').click(function(){                    
            $("#CMSKHSkillId").attr("readonly", false);                
        ...
        $(document).on('click', '.edit', function(){
            var id = $(this).attr('id');
            $.ajax({
                url: "{{route('skills.fetchData')}}",
                method: 'GET',
                data: {id:id},
                dataType: 'json',
                success: function(data)
                {
                    var lastCreated = formattedDate(new Date(data.CMSKHLastCreated));
                    var lastUpdated = formattedDate(new Date(data.CMSKHLastUpdated));
                    $('#CMSKHUniqueId').val(data.CMSKHUniqueId);
                    $('#CMSKHSkillId').val(data.CMSKHSkillId);
                    $('#CMSKHDesc1').val(data.CMSKHDesc1);
                    $('#CMSKHDesc2').val(data.CMSKHDesc2);
                    $('#CMSKHBiDesc').val(data.CMSKHBiDesc);
                    $('#CMSKHUser').val(data.CMSKHUser);
                    $('#CMSKHLastCreated').val(lastCreated);
                    $('#CMSKHLastUpdated').val(lastUpdated);
                    $("#CMSKHSkillId").attr("readonly", true);                       
        ...
        $(document).on('click', '.delete', function(){
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    $deleteMessage3SIS = fnSingleLevelDeleteConfirmation($modalTitle, data.CMSKHSkillId, data.CMSKHDesc1);   
        ...
        $('#Undelete_Data').click(function(){                    
            ...
            destroy: true,                    
            // CopyChange                    
            "ajax": "{{ route('skills.browserDeletedRecords')}}",
                "columns":[
                    // CopyChange
                    {data: "CMSKHSkillId"},
                    {data: "CMSKHDesc1"},
        ...
        $(document).on('click', '.restore', function(){
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    $restoreMessage3SIS = fnSingleLevelRestoreConfirmation($modalTitle, data.CMSKHSkillId, '');   
            ...
            $(document).on('click', '.confirmrestore', function(){
                $.ajax({
                    url:"{{route('skills.restoreDeletedRecords')}}",
                    mehtod:"get",
                    data:{id:DeletedUniqueId},
                    success:function(data)
                    {                       
