                                ...
                                <div class="container-fluid">
                                        <div class="table-responsive">
                                            <table id="html-extension3SIS" class="table table-hover non-hover" style="width:100%">
                                                <thead>
                                                    <tr>                                                                
                                                        <th title="Certificates Master">ID</th>
                                                        <th>Certificates</th>
                                                        <th>Desc1</th>
                                                        <th>Action</th>
                                                        <th style="visibility: hidden;">Unique Id</th>
                ...
                <div class="modal-dialog modal-dialog-centered 3SISPro-modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                        ...
                        <form id="singleLevelDataEntryForm" autocomplete="off" 
                            method="post" action="{{ route('certificates.postData') }}">
                                        ...
                                        <div class="tab-pane fade show active" id="animated-underline-home" role="tabpanel" 
                                            ...
                                            <div class="container-fluid">
                                                <div class="form-group">
                                                    <input type="hidden" name="CMCTHUniqueId" id="CMCTHUniqueId" class="form-control" />
                                                </div>
                                                <div class="row mt-0">
                                                    <div class="col-md-6">
                                                        <div class="form-group">
                                                            <label>Certificates Id</label>
                                                            <span class="error-text CMCTHCertificateId_error text-danger" style="float: right;"></span>
                                                            <input type="text" name="CMCTHCertificateId" id="CMCTHCertificateId" class="form-control few-options" maxlength="10" placeholder="Certificates Id" style="opacity: 1;" />
                                                        </div>
                                                    </div>
                                                    <div class="col-md-6">
                                                        <div class="form-group">
                                                            <label>Description</label>
                                                            <span class="error-text CMCTHDesc1_error text-danger" style="float: right;"></span>
                                                            <input type="text" name="CMCTHDesc1" id="CMCTHDesc1" class="form-control few-options" maxlength="100" placeholder="Certificates Description" />
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="form-group">
                                                    <label>Description2</label>
                                                    <span class="error-text CMCTHDesc2_error text-danger" style="float: right;"></span>
                                                    <textarea
                                                        name="CMCTHDesc2"
                                                        id="CMCTHDesc2"
                                                        ...
                                                        name="alloptions"
                                                        id="alloptions1"
                                                        placeholder="Certificates Description2"
                                                ...
                                                <div class="form-group">
                                                    <label>BI Desc</label>
                                                    <span class="error-text CMCTHBiDesc_error text-danger" style="float: right;"></span>
                                                    <input type="text" name="CMCTHBiDesc" id="CMCTHBiDesc" class="form-control few-options" maxlength="100" placeholder="BI Description" />
                                                </div>
                                            </div>
                                        </div>
...
<script>
    $(document).ready(function(){
        ...
        $('#html5-extension3SIS').DataTable( {
            ...
            processing: true,
            serverSide: true,                
            "ajax": "{{ route('certificates.browserData')}}",        
            "columns":[                    
                {data: "CMCTHCertificateId"},
                {data: "CMCTHDesc1"},
                {data: "CMCTHDesc2"},                
                {data: "action", orderable:false, searchable: false},
                {data: "CMCTHUser", "visible": false},
                {data: "CMCTHLastUpdated", "visible": false},
                {data: "CMCTHUniqueId", "visible": false},
            ],
            "columnDefs": [
                { "width": "10%", "targets": 0 },
                { "width": "25%", "targets": 1 },
        ...
        $('#add_Data').click(function(){                    
            $("#CMCTHCertificateId").attr("readonly", false);                
        ...
        $(document).on('click', '.edit', function(){
            var id = $(this).attr('id');
            $.ajax({
                url: "{{route('certificates.fetchData')}}",
                method: 'GET',
                data: {id:id},
                dataType: 'json',
                success: function(data)
                {
                    var lastCreated = formattedDate(new Date(data.CMCTHLastCreated));
                    var lastUpdated = formattedDate(new Date(data.CMCTHLastUpdated));
                    $('#CMCTHUniqueId').val(data.CMCTHUniqueId);
                    $('#CMCTHCertificateId').val(data.CMCTHCertificateId);
                    $('#CMCTHDesc1').val(data.CMCTHDesc1);
                    $('#CMCTHDesc2').val(data.CMCTHDesc2);
                    $('#CMCTHBiDesc').val(data.CMCTHBiDesc);
                    $('#CMCTHUser').val(data.CMCTHUser);
                    $('#CMCTHLastCreated').val(lastCreated);
                    $('#CMCTHLastUpdated').val(lastUpdated);
                    $("#CMCTHCertificateId").attr("readonly", true);                       
        ...
        $(document).on('click', '.delete', function(){
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    $deleteMessage3SIS = fnSingleLevelDeleteConfirmation($modalTitle, data.CMCTHCertificateId, data.CMCTHDesc1);   
            ...
            $(document).on('click', '.confirm', function(){
                $.ajax({
                    url:"{{route('certificates.deleteData')}}",
                    mehtod:"get",
                    data:{id:UniqueId},
                    success:function(data)
                    {
        ...
        $('#Undelete_Data').click(function(){                    
            ...
            destroy: true,                    
            // CopyChange                    
            "ajax": "{{ route('certificates.browserDeletedRecords')}}",
                "columns":[
                    // CopyChange
                    {data: "CMCTHCertificateId"},
                    {data: "CMCTHDesc1"},
        ...
        $(document).on('click', '.restore', function(){
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    $restoreMessage3SIS = fnSingleLevelRestoreConfirmation($modalTitle, data.CMCTHCertificateId, '');   
            ...
            $(document).on('click', '.confirmrestore', function(){
                $.ajax({
                    url:"{{route('certificates.restoreDeletedRecords')}}",
                    mehtod:"get",
                    data:{id:DeletedUniqueId},
                    success:function(data)
                    {                       
