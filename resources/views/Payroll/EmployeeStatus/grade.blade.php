                ...
                <div class="modal-dialog modal-dialog-centered 3SISPro-modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                        ...
                        <form id="singleLevelDataEntryForm" autocomplete="off" 
                            method="post" action="{{ route('grade.postData') }}">
                                        ...
                                        <div class="tab-pane fade show active" id="animated-underline-home" role="tabpanel" 
                                            ...
                                            <div class="container-fluid">
                                                <div class="form-group">
                                                    <input type="hidden" name="ESGRHUniqueId" id="ESGRHUniqueId" class="form-control" />
                                                </div>
                                                <div class="row mt-0">
                                                    <div class="col-md-6">
                                                        <div class="form-group">
                                                            <label>Grade Id</label>
                                                            <span class="error-text ESGRHGradeId_error text-danger" style="float: right;"></span>
                                                            <input type="text" name="ESGRHGradeId" id="ESGRHGradeId" class="form-control few-options" maxlength="10" placeholder="Grade Id" style="opacity: 1;" />
                                                        </div>
                                                    </div>
                                                    <div class="col-md-6">
                                                        <div class="form-group">
                                                            <label>Description</label>
                                                            <span class="error-text ESGRHDesc1_error text-danger" style="float: right;"></span>
                                                            <input type="text" name="ESGRHDesc1" id="ESGRHDesc1" class="form-control few-options" maxlength="100" placeholder="Grade Description" />
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="form-group">
                                                    <label>Description2</label>
                                                    <span class="error-text ESGRHDesc2_error text-danger" style="float: right;"></span>
                                                    <textarea
                                                        name="ESGRHDesc2"
                                                        id="ESGRHDesc2"
                                                        ...
                                                        name="alloptions"
                                                        id="alloptions1"
                                                        placeholder="Grade Description2"
                                                ...
                                                <div class="form-group">
                                                    <label>BI Desc</label>
                                                    <span class="error-text ESGRHBiDesc_error text-danger" style="float: right;"></span>
                                                    <input type="text" name="ESGRHBiDesc" id="ESGRHBiDesc" class="form-control few-options" maxlength="100" placeholder="BI Description" />
                                                </div>
                                            </div>
                                        </div>
                                        <div class="tab-pane fade" id="animated-underline-profile" role="tabpanel" 
                                            ...
                                            <div class="media">
                                                <div class="media-body">
                                                    <div class="form-group">
                                                        <label> User</label>
                                                        <input type="text" name="ESGRHUser" id="ESGRHUser" 
                                                        class="form-control col-sm-6" readonly/>
                                                    </div>
                                                    <div class="form-group">
                                                        <label> Created Date</label>
                                                        <input type="date" name="ESGRHLastCreated" id="ESGRHLastCreated" 
                                                        class="form-control col-sm-6" readonly/>
                                                    </div>
                                                    <div class="form-group">
                                                        <label> Updated Date</label>
                                                        <input type="date" name="ESGRHLastUpdated" id="ESGRHLastUpdated" 
                                                        class="form-control col-sm-6" readonly />
                                                    </div>                                                        
                                                </div>
                                            </div>
...
<script>
    $(document).ready(function(){
        ...
        $('#html5-extension3SIS').DataTable( {
            ...
            processing: true,
            serverSide: true,                
            "ajax": "{{ route('grade.browserData')}}",        
            "columns":[                    
                {data: "ESGRHGradeId"},
                {data: "ESGRHDesc1"},
                {data: "ESGRHDesc2"},                
                {data: "action", orderable:false, searchable: false},
                {data: "ESGRHUser", "visible": false},
                {data: "ESGRHLastUpdated", "visible": false},
                {data: "ESGRHUniqueId", "visible": false},
            ],
            "columnDefs": [
                { "width": "10%", "targets": 0 },
                { "width": "25%", "targets": 1 },
        ...
        $('#add_Data').click(function(){                    
            $("#ESGRHGradeId").attr("readonly", false);                
        ...
        $(document).on('click', '.edit', function(){
            ...
            $.ajax({
                ...
                {
                    var lastCreated = formattedDate(new Date(data.ESGRHLastCreated));
                    var lastUpdated = formattedDate(new Date(data.ESGRHLastUpdated));
                    $('#ESGRHUniqueId').val(data.ESGRHUniqueId);
                    $('#ESGRHGradeId').val(data.ESGRHGradeId);
                    $('#ESGRHDesc1').val(data.ESGRHDesc1);
                    $('#ESGRHDesc2').val(data.ESGRHDesc2);
                    $('#ESGRHBiDesc').val(data.ESGRHBiDesc);
                    $('#ESGRHUser').val(data.ESGRHUser);
                    $('#ESGRHLastCreated').val(lastCreated);
                    $('#ESGRHLastUpdated').val(lastUpdated);
                    $("#ESGRHGradeId").attr("readonly", true);                       
        ...
        $(document).on('click', '.delete', function(){
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    $deleteMessage3SIS = fnSingleLevelDeleteConfirmation($modalTitle, data.ESGRHGradeId, data.ESGRHDesc1);   
        ...
        $('#Undelete_Data').click(function(){                    
            ...
            destroy: true,                    
            // CopyChange                    
            "ajax": "{{ route('grade.browserDeletedRecords')}}",
                "columns":[
                    // CopyChange
                    {data: "ESGRHGradeId"},
                    {data: "ESGRHDesc1"},
        ...
        $(document).on('click', '.restore', function(){
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    $restoreMessage3SIS = fnSingleLevelRestoreConfirmation($modalTitle, data.ESGRHGradeId, '');   
            ...
            $(document).on('click', '.confirmrestore', function(){
                $.ajax({
                    url:"{{route('grade.restoreDeletedRecords')}}",
                    mehtod:"get",
                    data:{id:DeletedUniqueId},
                    success:function(data)
                    {                       
