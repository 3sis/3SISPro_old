                                ...
                                <div class="container-fluid">
                                        <div class="table-responsive">
                                            <table id="html-extension3SIS" class="table table-hover non-hover" style="width:100%">
                                                <thead>
                                                    <tr>                                                                
                                                        <th title="Designation Master">ID</th>
                                                        <th>Designation</th>
                                                        <th>Desc1</th>
                                                        <th>Action</th>
                                                        <th style="visibility: hidden;">Unique Id</th>
                ...
                <div class="modal-dialog modal-dialog-centered 3SISPro-modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                        ...
                        <form id="singleLevelDataEntryForm" autocomplete="off" 
                            method="post" action="{{ route('designation.postData') }}">
                                        ...
                                        <div class="tab-pane fade show active" id="animated-underline-home" role="tabpanel" 
                                            ...
                                            <div class="container-fluid">
                                                <div class="form-group">
                                                    <input type="hidden" name="ESDEHUniqueId" id="ESDEHUniqueId" class="form-control" />
                                                </div>
                                                <div class="row mt-0">
                                                    <div class="col-md-6">
                                                        <div class="form-group">
                                                            <label>Designation Id</label>
                                                            <span class="error-text ESDEHDesignationId_error text-danger" style="float: right;"></span>
                                                            <input type="text" name="ESDEHDesignationId" id="ESDEHDesignationId" class="form-control few-options" maxlength="10" placeholder="Designation Id" style="opacity: 1;" />
                                                        </div>
                                                    </div>
                                                    <div class="col-md-6">
                                                        <div class="form-group">
                                                            <label>Description</label>
                                                            <span class="error-text ESDEHDesc1_error text-danger" style="float: right;"></span>
                                                            <input type="text" name="ESDEHDesc1" id="ESDEHDesc1" class="form-control few-options" maxlength="100" placeholder="Designation Description" />
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="form-group">
                                                    <label>Description2</label>
                                                    <span class="error-text ESDEHDesc2_error text-danger" style="float: right;"></span>
                                                    <textarea
                                                        name="ESDEHDesc2"
                                                        id="ESDEHDesc2"
                                                ...
                                                <div class="form-group">
                                                    <label>BI Desc</label>
                                                    <span class="error-text ESDEHBiDesc_error text-danger" style="float: right;"></span>
                                                    <input type="text" name="ESDEHBiDesc" id="ESDEHBiDesc" class="form-control few-options" maxlength="100" placeholder="BI Description" />
                                                </div>
                                            </div>
                                        </div>
                                        <div class="tab-pane fade" id="animated-underline-profile" role="tabpanel" 
                                            ...
                                            <div class="media">
                                                <div class="media-body">
                                                    <div class="form-group">
                                                        <label> User</label>
                                                        <input type="text" name="ESDEHUser" id="ESDEHUser" 
                                                        class="form-control col-sm-6" readonly/>
                                                    </div>
                                                    <div class="form-group">
                                                        <label> Created Date</label>
                                                        <input type="date" name="ESDEHLastCreated" id="ESDEHLastCreated" 
                                                        class="form-control col-sm-6" readonly/>
                                                    </div>
                                                    <div class="form-group">
                                                        <label> Updated Date</label>
                                                        <input type="date" name="ESDEHLastUpdated" id="ESDEHLastUpdated" 
                                                        class="form-control col-sm-6" readonly />
                                                    </div>                                                        
                                                </div>
                                            </div>
...
<script>
    $(document).ready(function(){
        ...
        $('#html5-extension3SIS').DataTable( {
            ...
            processing: true,
            serverSide: true,                
            "ajax": "{{ route('designation.browserData')}}",        
            "columns":[                    
                {data: "ESDEHDesignationId"},
                {data: "ESDEHDesc1"},
                {data: "ESDEHDesc2"},                
                {data: "action", orderable:false, searchable: false},
                {data: "ESDEHUser", "visible": false},
                {data: "ESDEHLastUpdated", "visible": false},
                {data: "ESDEHUniqueId", "visible": false},
            ],
            "columnDefs": [
                { "width": "10%", "targets": 0 },
                { "width": "25%", "targets": 1 },
        ...
        $('#add_Data').click(function(){                    
            $("#ESDEHDesignationId").attr("readonly", false);                
        ...
        $(document).on('click', '.edit', function(){
            var id = $(this).attr('id');
            $.ajax({
                url: "{{route('designation.fetchData')}}",
                method: 'GET',
                data: {id:id},
                dataType: 'json',
                success: function(data)
                {
                    var lastCreated = formattedDate(new Date(data.ESDEHLastCreated));
                    var lastUpdated = formattedDate(new Date(data.ESDEHLastUpdated));
                    $('#ESDEHUniqueId').val(data.ESDEHUniqueId);
                    $('#ESDEHDesignationId').val(data.ESDEHDesignationId);
                    $('#ESDEHDesc1').val(data.ESDEHDesc1);
                    $('#ESDEHDesc2').val(data.ESDEHDesc2);
                    $('#ESDEHBiDesc').val(data.ESDEHBiDesc);
                    $('#ESDEHUser').val(data.ESDEHUser);
                    $('#ESDEHLastCreated').val(lastCreated);
                    $('#ESDEHLastUpdated').val(lastUpdated);
                    $("#ESDEHDesignationId").attr("readonly", true);                       
        ...
        $(document).on('click', '.delete', function(){
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    $deleteMessage3SIS = fnSingleLevelDeleteConfirmation($modalTitle, data.ESDEHDesignationId, data.ESDEHDesc1);   
            ...
            $(document).on('click', '.confirm', function(){
                $.ajax({
                    url:"{{route('designation.deleteData')}}",
                    mehtod:"get",
                    data:{id:UniqueId},
                    success:function(data)
                    {
        ...
        $('#Undelete_Data').click(function(){                    
            ...
            destroy: true,                    
            // CopyChange                    
            "ajax": "{{ route('designation.browserDeletedRecords')}}",
                "columns":[
                    // CopyChange
                    {data: "ESDEHDesignationId"},
                    {data: "ESDEHDesc1"},
        ...
        $(document).on('click', '.restore', function(){
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    $restoreMessage3SIS = fnSingleLevelRestoreConfirmation($modalTitle, data.ESDEHDesignationId, '');   
            ...
            $(document).on('click', '.confirmrestore', function(){
                $.ajax({
                    url:"{{route('designation.restoreDeletedRecords')}}",
                    mehtod:"get",
                    data:{id:DeletedUniqueId},
                    success:function(data)
                    {                       
